...
def test_token_commit_and_win(accounts):
    ...
    assert wolvercoin_contract.balanceOf(user) >= initial_balance + expected_winning or wolvercoin_contract.balanceOf(user) == initial_balance, "User balance should increase by the expected winning amount"
